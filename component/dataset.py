...
class PandaSFTDataset(Dataset):
    ...
    统一的数据处理dataset
    """
    def __init__(self, file, tokenizer, max_seq_length, template, cluster_file):
        ...
        logger.info('Loading centroid data from: {}'.format(cluster_file))
        self.centroids = torch.load(cluster_file).numpy()
    ...
    def __getitem__(self, index):
        ...
        question = self.user_format.format(content=data["instruction"], stop_token=self.tokenizer.eos_token)
        input_tokens = self.tokenizer.encode(question, add_special_tokens=False)
        answer = self.assistant_format.format(content=data["output"], stop_token=self.tokenizer.eos_token)
        ...
        # 获取emb, 计算relation score
        emb = data['emb']
        relate_score = self.centroids @ emb
        relate_score = relate_score.tolist()
