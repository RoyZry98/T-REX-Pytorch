...
def setup_everything():
    ...
    parser.add_argument("--train_args_file", type=str, default='./train_args/llama2-13b-trex.json', help="")
    parser.add_argument("--local_rank", type=int, help="")
    parser.add_argument("--PDdataset", default=False, action="store_true", help="")
    ...
    parser.add_argument("--moe_mode", type=str, required=True, help="\
                        none: traditional lora \
                        lora: lora-MoE \
                        rank1: rank1-MoE \
                        rank1_flex: N vs N expert match \
                        ")
    parser.add_argument("--rank1_flex_rank_allocation", nargs='+', type=int, default=None, help="lora_A's dim")
    parser.add_argument("--rank1_flex_topk", type=int, default=-1,help="top-k selection if select rank1_flex, where -1 refers to top-all")
    ...
    # write moe_mode to external cfg file for later import in lora.py
    external_cfg = {
    ...
    folder_name = os.path.basename(training_args.output_dir)
    path_without_deepest_folder = os.path.dirname(training_args.output_dir)
    folder_name = f"{formatted_time}_{folder_name}"
    training_args.output_dir = os.path.join(path_without_deepest_folder, folder_name)
...
def load_model(args, training_args):
    ...
    if args.train_mode == 'full':
        ...
        peft_config = LoraConfig(
            ...
            bias="none",
            task_type="CAUSAL_LM",
            modules_to_save=["omg","router",'binary_main_gate_weight']
        )
...
def load_sft_dataset(args, tokenizer):
    ...
    if 'chatglm2' in args.model_name_or_path.lower():
        ...
        if system_args.PDdataset:
            train_dataset = PandaSFTDataset(args.train_file, tokenizer, args.max_seq_length, template, system_args.cluster_file)
        else:
